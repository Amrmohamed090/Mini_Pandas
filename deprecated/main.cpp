...
public:
    ...
    DataFrame(vector<string>* col_names = nullptr, vector<int>* col_dtypes = nullptr) {
        ...
        if (col_names && col_dtypes) {
            ...
            for (size_t i = 0; i < columns_names.size(); i++) {
                ...
                if      (columns_dtypes[i] == 2) data[name] = vector<string>();
                else if (columns_dtypes[i] == 1) data[name] = vector<double>();
                else                             data[name] = vector<int>();
            }
        }
    }
    ...
    vector<CellValue> get_row(size_t index) {
        ...
        if (index < 0 || index >= static_cast<int>(num_rows)) {
    ...
    template<typename T>
    unordered_map<T, int> value_counts(string col_name) {
        unordered_map<T, int> counts;

        for (const auto& val : get<vector<T>>(data[col_name])) {
            counts[val]++;
        }
...
template<typename T>
void print_map(unordered_map<T, int> mp, string name = ""){
    cout<<" Map for "<<name<<"\n\n";
...
int main() {
    ...
    //1. The mean maximum speed for each player during Practice sessions.
    vector<bool> * filter = df.get_filter("Name", string("Jack"));
    vector<bool> * filter1 = df.get_filter("Session_Type", string("Practice"), filter);

    DataFrame filtered_df = df.where(filter1);
    print_map<string>(df.value_counts<string>("Name"));
    // // Print the first 10 rows
    filtered_df.print(10);
